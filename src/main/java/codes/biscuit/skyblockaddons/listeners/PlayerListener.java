...
public class PlayerListener {
	...
	private int     ticksSinceInventoryOpen = 0;
	...
	private Field fieldGuiChestUpperInventory;
	...
	/**
	 * Interprets the action bar to extract mana, health, and defence. Enables/disables mana/health prediction, and looks for mana usage messages in chat while predicting.
	 */
	...
	/**
	 * This blocks interaction with Ember Rods on your island, to avoid blowing up chests, and placing enchanted items such as enchanted lava buckets.
	 */
	@SubscribeEvent()
			...
			if (mc != null) { // Predict health every tick if needed.
				...
				if (timerTick == 20) { // Add natural mana every second (increase is based on your max mana).
					if (main.getRenderListener().isPredictMana()) {
						...
						if (getAttribute(Attribute.MANA) > getAttribute(Attribute.MAX_MANA)) { setAttribute(Attribute.MANA, getAttribute(Attribute.MAX_MANA)); }
					}
	...
	public void onItemTooltip(ItemTooltipEvent e) {
		...
		if (hoveredItem.hasTagCompound() && GuiScreen.isCtrlKeyDown() && main.getUtils().isCopyNBT()) {
			...
			String nbt = new Gson().toJson(hoveredItem.getTagCompound());
			try {
	...
	public void onPlayerTick(TickEvent.PlayerTickEvent e) {
		...
		if (screen instanceof GuiContainer) {
			if (ticksSinceInventoryOpen++ == 50) {
				postAuctionInventory(mc, (GuiContainer) screen);
			}
		} else {
			ticksSinceInventoryOpen = 0;
		}
	...
	public void postAuctionInventory(Minecraft mc, GuiContainer gui) {
		if (gui instanceof GuiChest) {
			GuiChest chest = (GuiChest) gui;

			if (fieldGuiChestUpperInventory == null) {
				fieldGuiChestUpperInventory = ReflectionHelper.findField(GuiChest.class, "upperChestInventory", "field_147016_v");
			}
